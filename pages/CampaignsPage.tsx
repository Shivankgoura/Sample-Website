...
const campaignsData = [
    ...
    { 
        ...
        category: "Photography",
        image: "https://picsum.photos/seed/campaign6/800/600",
        description: "A comprehensive product photography shoot for a new line of consumer electronics, focusing on clean lighting and detailed shots to highlight design and functionality.",
        tags: ["E-commerce", "Studio Lighting", "Product Styling"]
    },
];
...
const CampaignsPage: React.FC<CampaignsPageProps> = ({ onContactClick }) => {
    const [activeFilter, setActiveFilter] = useState<Category>('All');

    const filteredCampaigns = activeFilter === 'All'
        ? campaignsData
        : campaignsData.filter(p => p.category === activeFilter);
            ...
            <section className="relative pt-24 sm:pt-32 pb-12 sm:pb-16 text-center bg-gradient-to-b from-black via-gray-900 to-black">
                ...
                <div className="relative z-10 container mx-auto px-4">
                    ...
                    <p className="mt-6 text-lg md:text-xl text-white/80 max-w-3xl mx-auto">
                        Explore a selection of our successful campaigns, from cinematic ads to robust web platforms and intelligent automation solutions.
                    </p>
                </div>
            </section>
                ...
                <div className="container mx-auto px-4">
                    ...
                    >
                        {filteredCampaigns.map((campaign) => (
                            <motion.div 
                                key={campaign.id} 
                                className="group relative bg-black/40 border border-white/10 rounded-2xl overflow-hidden"
                                variants={itemVariants}
                            >
                                ...
                                <div className="p-4 md:p-6">
                                    <span className="text-xs bg-cyan-400/10 text-cyan-300 px-2 py-1 rounded-full">{campaign.category}</span>
                                    <h3 className="text-xl md:text-2xl font-bold text-white mt-4">{campaign.title}</h3>
                                    <p className="text-gray-300 mt-2 min-h-[4.5rem]">{campaign.description}</p>
                                    <div className="flex flex-wrap gap-2 mt-4">
                                        {campaign.tags.map(tag => (
                                            <span key={tag} className="text-xs bg-white/5 text-gray-400 px-2 py-1 rounded">{tag}</span>
                                        ))}
                                    </div>
                                    <a href="#" className="inline-flex items-center text-cyan-400 font-semibold mt-6 group-hover:text-white transition-colors">
                                        View Details
            ...
            <section className="py-12 sm:py-16 bg-black/20">
                <div className="container mx-auto px-4 text-center">
                    <motion.div
                         ...
                         variants={itemVariants}
                    >
                        <h2 className="font-heading text-4xl md:text-5xl font-bold text-white">Have a campaign in mind?</h2>
                        <p className="mt-4 text-lg text-gray-300 max-w-2xl mx-auto">
                           Let's bring your next big idea to life with creativity and intelligence.
                        </p>
                        <button
